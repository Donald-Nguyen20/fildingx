...
def build_vector_store_from_files(
    ...
    extract_content_fn,
    output_dir,
    source_label="picked_files",
    progress_cb=None,
):
    """
    Build vector store from a list of file paths (multi-file selection).
    Keep the same metadata format as build_vector_store(folder).
    """

    # Nếu file bạn đang có đã import các thứ này rồi thì không cần import lại.
    import os, time, json
    import numpy as np
    import faiss
    from sentence_transformers import SentenceTransformer
    ...
    # --- lọc file hợp lệ ---
    allowed_ext = {".pdf", ".docx", ".xlsx", ".pptx", ".txt", ".csv", ".md", ".html", ".json", ".xml"}
    files = []
    ...
    # --- root chung để rel_path nhìn đẹp ---
    try:
    ...
    # Nếu project bạn có ENV min chunk thì giữ; không có thì để default
    MIN_CHUNK_LEN = int(os.environ.get("RAG_MIN_CHUNK_LEN", "80"))
    ...
    for i, file_path in enumerate(files, start=1):
        ...
        text = normalize_text(raw) if "normalize_text" in globals() else (raw or "").strip()
        if not text:
            if progress_cb:
                progress_cb(int(i * 100 / total))
            continue

        # chunking: dùng chunk_text_sop nếu có, không có thì fallback cắt thô
        if "chunk_text_sop" in globals():
            chunks = chunk_text_sop(text, target_chars=900, hard_max=1300)
        else:
            chunks = [{"text": text, "section": "", "subsection": ""}]
        ...
        for cid, obj in enumerate(chunks):
            ...
            # ChunkMeta phải giống với build_vector_store(folder) của bạn
            metas.append(ChunkMeta(
                id=next_id,
                file_name=name,
                rel_path=rel_path,
    ...
    if not metas:
        raise RuntimeError("No text extracted from selected files.")

    # --- embed + FAISS ---
    # model_name: giữ đúng tên model bạn đang dùng trong build_vector_store(folder)
    ...
    base_path = common_root  # hoặc os.path.dirname(files[0])
    with open(os.path.join(output_dir, "base_path.txt"), "w", encoding="utf-8") as f:
        f.write(base_path)
