...
class BuildStoreWorkerFiles(QThread):
    ...
    def run(self):
        try:
            ...
            self.log.emit(f"Output dir: {self.output_dir}")
...
class VectorStoreDialog(QDialog):
    ...
    def pick_folder(self):
        ...
        # 2) Nếu user Cancel -> chuyển qua chọn FOLDER
    ...
    def start_build(self):
        ...
        # 1) Tạo output dir (y như code anh đang có)
        ...
        # (thường anh có disable nút + reset progress ở đây)
        self.btn_build.setEnabled(False)
        self.pb.setValue(0)
        self.log(f"Output folder created: {out_dir}")

        # 2) >>> DÁN ĐOẠN NÀY VÀO ĐÂY <<<
        # --- MODE 1: nếu đã chọn files (từ Browse) ---
        if getattr(self, "_picked_files", None):
            self.worker = BuildStoreWorkerFiles(self._picked_files, out_dir)

        # --- MODE 2: folder như cũ ---
        else:
        ...
        # 3) Nối signal + start worker (đoạn này anh đang có sẵn)
